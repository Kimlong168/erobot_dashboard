...
function CreateGuestImage() {
  ...
  const uploadFiles = async () => {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      const imageRef = ref(storage, `guestImages/${imageId}`);
      await uploadBytes(imageRef, images[i])
        .then(() => {
          // Get the download URL for the uploaded image
          getDownloadURL(imageRef)
            .then((downloadURL) => {
              console.log("guestImages image URL:", downloadURL);
              //store author and image to firestore

              const myCollection = collection(db, "guestImages");
              addDoc(myCollection, {
                name: images[i].name,
                url: downloadURL,
                imageId: imageId,
...
export default CreateGuestImage;

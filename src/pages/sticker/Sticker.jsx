...
const Sticker = () => {
  const { stickerList, setShowNotification } = useContext(DataContext);
  ...
  const notifyDeleting = (id, imageId) => {
    toast.error(
      <>
        <DeletingAlertBox
          deleteItemFucntion={() => {
            deleteItemFucntion(id, "stickers")
              .then((result) => {
  ...
  return (
    <Layout>
      ...
      <TableHead
        color="rgb(88,28,135)"
        title={`Stickers (${stickerList.length})`}
